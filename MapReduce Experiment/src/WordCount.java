...
public class WordCount {
	   ...
	   private static final String PRIMARY_KEY = "row_key";
		...
		public static class CustomPartitioner extends Partitioner<Text, IntWritable> {
			...
			public int getPartition(Text key, IntWritable value, int numReduceTasks) {
				...
				/* Don't create the SSMB/Partitioner objects each time this is called. Make it singleton or something.*/
				try {
					...
					Token tok = partitioner.getToken(buf).getToken();
					System.out.println(tok);

					//Iterate over map to find relevant list of endpoints for each token
					for (Map.Entry<Range<Token>, List<String>> entry : tok_ring.entrySet()) {
						Range<Token> range = entry.getKey();
						if(range.contains(tok)) {
							//Grouping changes for one key when endpoint list changes.
							//Using endpoint list[0] is more stable.
							String first_endpt = entry.getValue().get(0);
							return first_endpt.hashCode()%numReduceTasks;
						}
					}
					System.out.println("Error : Token Ring does not include token from Murmur3Partitioner");
					System.exit(127);
				} catch (IOException e) {
	...
	public static void main(String[] args) throws Exception {
        ...
        ConfigHelper.setInputInitialAddress(job.getConfiguration(), SEED_CASSANDRA_SERVER);
