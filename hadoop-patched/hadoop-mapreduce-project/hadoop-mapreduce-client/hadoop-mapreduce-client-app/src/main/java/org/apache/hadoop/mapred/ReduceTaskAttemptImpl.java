...
public class ReduceTaskAttemptImpl extends TaskAttemptImpl {
  ...
  public ReduceTaskAttemptImpl(TaskId id, int attempt,
    ...
    super(id, attempt, eventHandler, taskAttemptListener, jobFile, partition,
        conf, ReduceTaskAttemptImpl.getReducerLocalityHints(conf, partition), jobToken, credentials, clock,
        appContext);
    this.numMapTasks = numMapTasks;
  }
  private static String [] getReducerLocalityHints(JobConf conf, int partition) {
        Preconditions.checkArgument(partition >= 0, "The partition number passed is negative.");
        String[] reduceLocalityHints = conf.getStrings(MRJobConfig.MR_JOB_REDUCE_LOCALITY_HINTS);
        if (reduceLocalityHints != null) {
            if (partition < reduceLocalityHints.length) {
                reduceLocalityHints = new String[] { reduceLocalityHints[partition] };
            } else {
                reduceLocalityHints = new String[] {};
            }
        } else {
            reduceLocalityHints = new String[] {};
        }
        return reduceLocalityHints;
  }
