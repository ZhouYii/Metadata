...
public class TestTaskAttempt{
  ...
  public void testSingleRackRequest() throws Exception {
    ...
    @Test
    public void testReducerLocalityHints() throws Exception {
        TaskAttemptImpl.RequestContainerTransition rct =
                new TaskAttemptImpl.RequestContainerTransition(false);
        ...
        TaskAttemptImpl mockTaskAttempt =
                 createReduceTaskAttemptImplForTest(eventHandler, hosts, 0);
        TaskAttemptEvent mockTAEvent = mock(TaskAttemptEvent.class);
        rct.transition(mockTaskAttempt, mockTAEvent);
        ArgumentCaptor<Event> arg = ArgumentCaptor.forClass(Event.class);
        verify(eventHandler, times(2)).handle(arg.capture());
  ...
  private TaskAttemptImpl createMapTaskAttemptImplForTest(
    ...
    private TaskAttemptImpl createReduceTaskAttemptImplForTest(
        EventHandler eventHandler, String[] hosts, int partition) {
        ...
        Path jobFile = mock(Path.class);
        JobConf jobConf = new JobConf();
        jobConf.setStrings(MRJobConfig.MR_JOB_REDUCE_LOCALITY_HINTS, hosts);
        OutputCommitter outputCommitter = mock(OutputCommitter.class);
        TaskAttemptImpl taImpl = new ReduceTaskAttemptImpl(taskId, 1, eventHandler, jobFile, partition,
                              1, jobConf, taListener, outputCommitter, null,
                              null, new SystemClock(), null);
        return taImpl;
    }
